...
exports.handler = async (event) => {
  try {
    ...
    const [appointmentsRaw, customersRaw, vehiclesRaw] = await Promise.all([
      safeGetValues(sheets, spreadsheetId, "Appointments!A:J"),
      safeGetValues(sheets, spreadsheetId, "Customers!A:F"),
      safeGetValues(sheets, spreadsheetId, "Vehicles!A:F"),
    ]);

    const appointments = appointmentsRaw.slice(1).map((r) => ({
      id: r[0], customer_id: r[1], vehicle_id: r[2], date: r[3],
      time_slot: r[4], services: r[5], source: r[6], status: r[7],
      notes: r[8], admin_notes: r[9],
    }));

    const customers = customersRaw.slice(1).reduce((acc, r) => {
      acc[r[0]] = { id: r[0], first_name: r[1], last_name: r[2], phone: r[3], email: r[4], notes: r[5] };
      return acc;
    }, {});

    const vehicles = vehiclesRaw.slice(1).reduce((acc, r) => {
